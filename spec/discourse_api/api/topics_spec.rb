...
describe DiscourseApi::API::Topics do
  ...
  describe "#topic_set_user_notification" do
    before do
      stub_post("#{host}/t/1/notifications").to_return(body: fixture("notification_success.json"), headers: { content_type: "application/json" })
    end

    it "makes the post request" do
      response = subject.topic_set_user_notification id: 1, notification_level: 3
      expect(a_post("#{host}/t/1/notifications")).to have_been_made
